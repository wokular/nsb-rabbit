...
These functions will help set up the connection to the NSB server, send messages, 
and receive messages. These will be used by the following classes and functions.
*/
...
*/
{
    ...
    printf("HEADER - Type %s (%d) Len %u srcid %u dstid %u msgid %u\n",
        OH_MSG_TYPES_STR(oh->type), oh->type, oh->len,
...
oh_deliver_msg_ack_t * recv_oh_deliver_msg_ack(int sock)
{
    ...
    printf("Received returnCode=%s (%d)\n",
        ERROR_CODE_STR(oh_ack->returnCode), oh_ack->returnCode);
...
nsbHeaderAndData_t NSBConnector::receiveMessageFromNsbServer(std::string sourceAddress){
    ...
    if (DEBUG) printf("Sending request for message.\n");
    nsbSendMsg(sock, &header, sizeof(oh_header_t));
...
//    uint32_t sourceAddress_u32 = htonl(inet_addr(sourceAddress.c_str()));
//    uint32_t destinationAddress_u32 = htonl(inet_addr(destinationAddress.c_str()));
    ...
    header->type = OH_DELIVER_MSG,
    header->len = msgLen,
    header->dstid = destinationAddress,
    header->msgid = msgId,
    header->srcid = sourceAddress;
    // Copy message into packet.
    memcpy((char *)pkt + sizeof(oh_header_t), message, msgLen);
    // This message does not have any additional data, so we can send the header directly.
...
// Define a function that receives a message and immediately notifies the server that it was delivered.
int receiveAndNotify(NSBConnector *connector, std::string address){
    ...
    if (DEBUG) std::cout << "Message: " << std::string((char *)message.data, min) << "..." << std::endl;
    ...
    if (msgLen==0){
        ...
        if (DEBUG) std::cout << "There is no message! No call to notify server" <<std::endl;
        return -1;
    }
    else{
        // Notify the server that the message was delivered.
        if (ACTIVE_DEBUG) std::cout << "Notifying server that message was delivered." << std::endl;
        int result = connector->notifyDeliveryToNsbServer(msgLen,message.header.srcid , message.header.dstid, msgId, message.data);
...
// Lambda function receiveNotifyThread that runs the exiswting receiveAndNotify function on a given address in a while loop.
// void receiveNotifyThread(std::string address){
//     while (true){
...
int main(int argc, char *argv[]) {
    ...
    This section of the main function will be used to test the functions by sending and 
    notifying the delivery of a single message from one address to another.
    */
    #if 0
    // Receive a message from the server.
    ...
    This (much cooler) section of the main function will be used to run a ghost network 
    simulator client that will always pick up and immediately deliver messages. This can 
    be utilized to test the other modules, such as the application client or the 
    */
    #if 1
    ...
    // Create a list of IP addresses here (in this case, from 10.0.0.1 to 10.0.0.9)
