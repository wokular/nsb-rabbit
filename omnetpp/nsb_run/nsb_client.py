...
class NSBApplicationClient:
    def __init__(self):
        ...
        self.sock.connect((NSB_SERVER_ADDR, nsbp.PORT))
...
async def test_sender(connector : NSBApplicationClient, aliases : list, auto=False, rate=None):
    while True:
...
async def main_manual(map_file_name):
    ...
    await asyncio.gather(
        ...
        test_sender(connector, aliases)
    )

async def main_auto(map_file_name, rate):
    # Get list of addresses from aliasmap.txt.
    with open(map_file_name, "r") as f:
        lines = f.readlines()
    # Create a dictionary of aliases.
    ...
    await asyncio.gather(
        test_receiver(connector, aliases),
        test_sender(connector, aliases, auto=True, rate=rate)
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        """
        if not args.auto:
            asyncio.run(main_manual(map_file_name))
        else:
            asyncio.run(main_auto(map_file_name, args.rate))
